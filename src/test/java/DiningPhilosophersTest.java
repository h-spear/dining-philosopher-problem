...
public class DiningPhilosophersTest {
    ...
    private static final long RUNNING_TIME = 60 * 1000;
    ...
    @Test
    @DisplayName("순환대기 예방: 해시값이 큰 포크를 먼저 잡도록하면 데드락이 발생하지 않는다.")
    void diningPhilosophers_CircularWaitPreventionHashCode() {
        for (int i = 0; i < N; ++i) {
            philosophers.add(new Philosopher_CircularWaitPreventionHashCode(i, forks.get(i), forks.get((i + 1) % N)));
        }
        for (int i = 0; i < N; ++i) {
            philosophers.get(i).start();
        }
